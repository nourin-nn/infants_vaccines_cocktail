...
# ===================================
if (TRUE) {
  genes <- c("FCGR3A", "S100A4",
             "XCL1", "XCL2",  "NCAM1", "NCR1",
             "GZMA", "GZMB", "GZMH", 
             "NKG7", "KLRD1", "KLRB1", "PRF1", 
             "GNLY", "KLRF1", 
             "ISG15", "XAF1",  "IRF7" , "STAT1","ISG20",
             "HLA-A", "HLA-B", "HLA-C"
  ) 
...
# find markers
if (TRUE) {
  top_genes <- all_markers_nkc %>%
    dplyr::filter(!gene %in% c("COTL1", "CCL3", "CCL4")) %>%
...
if (TRUE) {
  ...
  my_row_hm <- data.frame(Cluster = my_row_hm) %>%
    dplyr::mutate(Cluster = case_when(Cluster == 2 ~ subC_levels[2],
                                      Cluster == 1 ~ subC_levels[1]))
  # ===================================
