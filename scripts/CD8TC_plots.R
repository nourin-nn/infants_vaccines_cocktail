...
if (TRUE) {
  ...
  p <- ggplot() +
    ...
    geom_text(data = filter(db_plot, frac > 0.05),
              aes(x = time_point, y = frac, fill = sub_seurat_clusters, label = paste0(round(frac, 3)*100, "%"), size = frac),
              fontface = "bold", position = position_stack(vjust = 0.5, reverse = TRUE)) +
...
# ===================================
if (TRUE) {
  genes <- c("CCR7", "LEF1", "SELL",  # naïve T-cell markers
             "S100A4", "IL7R", "CD27", "TCF7", 
             "CD69", #  activation markers
             "TRDC", "TRGC2", "TRG-AS1", # CD8 gamma-delta T
             "CX3CR1", "FGFBP2", "FCGR3A", # CD8+ recently-activated effector-memory T-cells (Temra)
             "PRF1", "KLRG1", "GZMA", "GZMB", "GZMH", # Cytotoxicity-related markers
             "ISG15", "XAF1", "STAT1",
             "HLA-A", "HLA-B", "HLA-C")
  ...
  p <- DotPlot(sobj,
    ...
    theme(panel.grid = element_blank(),
          ...
          legend.spacing = grid::unit(25, "points")) +
...
if (TRUE) {
  ...
  #- Cytotoxicity_NK_Cells
  genes_ctx <- unique(mod$gene[grepl("Cytotoxicity_",mod$ind)])
  length(genes_ctx)
  sobj_sc <- AddModuleScore(
    object = sobj_sc,
    assay = "RNA", 
    features = list(genes_ctx),
    name = 'cytotoxicity'
  )
  ...
  scrs <- as.numeric(sobj_sc@meta.data$cytotoxicity1)
  correlations <- apply(as.matrix(sobj_sc@assays$RNA@data), 1, 
                        function(x){ cor(scrs, x) })
  top_ctx_genes_scrs <- names(head(sort(correlations[genes_ctx], decreasing = T), 10)) # run this twice for top 10 by 1:5 and 6:10
}
# ===================================
if (TRUE) {
  gg_titles <- c("Interferon", "Cytotoxicity")
  x_lim <- list(c(-1,2),
                c(-1,2))
  gg_means <- list(gg_means <- sobj_sc@meta.data %>%
                   ...
                   gg_means <- sobj_sc@meta.data %>%
                     dplyr::group_by(sub_seurat_clusters) %>%
                     dplyr::summarise(means = mean(cytotoxicity1))
  )

  p <- RidgePlot(sobj_sc, 
                 features = c("interferon1", "cytotoxicity1"), 
                 assay = "RNA", slot = "data",
...
if (TRUE) {
  db_plot <- sobj_sc@meta.data %>%
    tibble::rownames_to_column(var = "barcode") %>%
    dplyr::select(barcode, sub_seurat_clusters, time_point, interferon1, cytotoxicity1) %>%
    ...
    dplyr::mutate(time_point = factor(time_point, levels = time_points, ordered = T),
                  module = case_when(module == "interferon1" ~ "Interferon",
                                     module == "cytotoxicity1" ~ "Cytotoxicity"),
  ...
  p <- ggplot(db_plot, 
    ...
    geom_vline(xintercept = seq(1.5, 2.5, 1), linetype = 2, size= 0.35, alpha = 0.65) +
    scale_color_manual(name = "",
                       values = tp_colrs) +
    scale_fill_manual(name = "",
                      values = tp_colrs) +
...
if (TRUE) {
  ...
  genes_df <- FetchData(sobj, genes, slot = "data") %>%
    ...
    dplyr::ungroup()   
  stopifnot(length(unique(genes_df$gene)) == length(genes))
}
# ===================================
if (TRUE) {
  genes_mtx <- genes_df %>%
  ...
  my_row_hm <- data.frame(Cluster = my_row_hm) %>%
    dplyr::mutate(Cluster = case_when(Cluster == 3 ~ subC_levels[3],
...
if (TRUE) {
  ...
  p <- ggplot(db,
    ...
    theme(plot.title = element_text(size = 11, hjust = 0.5, face = "bold"),
          axis.text.x = element_text(size = 18, face = "bold", angle = 25, hjust = 1, vjust = 1),
...
if (TRUE) {
  ...
  dotplot <- ggplot(db_plot,
    ...
    theme(legend.position = "right",
          ...
          legend.text = element_text(size = 14),
          axis.text.x = element_text(size = 15, face = "bold", angle = 30, hjust = 1, vjust = 1),
          strip.text = element_text(size = 20, face = "bold"),
          axis.text.y = element_text(size = 15, face = "bold"),
          axis.title = element_text(size = 20, face = "bold")) +
    geom_point(shape = 21, color = "black") + 
    scale_size(name = "Jaccard\nIndex",
               range = c(1,7)) + # , breaks = seq(0,1,0.02)
...
# ===================================
if (TRUE) {
  genes <- unique(deg.df$gene[deg.df$gene %in% c(genes_ifn, genes_ctx)])
  print(length(genes))
  # ===================================
  ...
  genes_mtx <- genes_mtx[rownames(genes_mtx) != "CD8A", ]
  # ===================================
  ...
  my_row_hm_1 <- sapply(rownames(genes_mtx), function(x){
    ...
    if (x %in% genes_ctx) return("Cytotoxicity")
  ...
  my_row_hm_2 <- sapply(rownames(genes_mtx), function(x){
    ...
    if (x %in% genes_ctx) return(mod$module[id][as.character(mod$signiture[id]) == "Cytotoxicity"][1])
  }, USE.NAMES = FALSE)
  ...
  for (regtype in c("up", "down")) {
    for (tp in time_points[2:3]) {
      ...
      p <- ggVennDiagram(genes_subs_ls, 
                         category.names = substr(names(genes_subs_ls), 1, 5),
                         set_size = 0,
