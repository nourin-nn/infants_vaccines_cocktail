...
# ===================================
# find markers
table(all_markers_cd4tc$sub_seurat_clusters)
if (TRUE) {
  top_genes <- all_markers_cd4tc %>%
...
if (TRUE) {
  ...
  my_row_hm <- data.frame(Cluster = my_row_hm) %>%
    dplyr::mutate(Cluster = case_when(Cluster == 5 ~ subC_levels[3],
                                      Cluster == 4 ~ subC_levels[5],
...
# ===================================
if (TRUE) {
  genes <- c("CCR7", "LEF1", "SELL", # naive
             "S100A4", "IL7R", # memory
             "CD27", "FOS", "TCF7", 
             "KLRB1", "ZBTB16", "CCR6" , #MAIT T cell
             "GZMA", "GZMB","GZMH","GZMK", "GZMM",
             "KLRG1", # effector- memory functions
             "RUNX3", # TH1-like transcription factors
             "FOXP3", "TIGIT", "IL2RA", "IKZF2", # T-reg
             "HLA-A", "HLA-B", "HLA-C",
             "ISG15", 
             "IFI6", "IFI44L", "IFI44", 
             "TBX21", # TH_1
...
if (TRUE) {
  db_plot <- gene_overlap_test %>%
    dplyr::filter(p_val < 0.05, !signiture %in% c("TBD", "Plasma_Cells")) %>%
...
if (TRUE) {
  ...
  top_ifn_genes_scrs <- names(head(sort(correlations[genes_ifn], decreasing = T), 10)) 
  stopifnot(all(top_ifn_genes_scrs %in% genes_ifn))
}
# ===================================
if (TRUE) {
...
if (TRUE) {
  ...
  pt <- as.data.frame(slingPseudotime(sds)) %>%
    tibble::rownames_to_column(var="barcode") %>%
    dplyr::rename(Trajectory_3 = curve1,
                  Trajectory_1 = curve2,
                  Trajectory_2 = curve3) %>%
